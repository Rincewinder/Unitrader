...
-- section == gJLPUniTrader_uiconfig_gMain_map
-- param == { 0, 0 , Entity, componenttype, component [, history] [, lastchild] [, mode, modeparam] }
...
local menu = {
	name = "gJLPUniTrader_uiconfig_gMain_map",
...
function menu.onShowMenu()
	...
	if menu.param2 == nil or menu.param2[1] then
		...
		menu.history = menu.param[6] or {}
		for _, entry in ipairs(menu.history) do
			entry[1] = ConvertIDTo64Bit(entry[1])
		end
		table.insert(menu.history, { ConvertStringTo64Bit(tostring(menu.component)), menu.componenttype })
		menu.lastchild = ConvertIDTo64Bit(menu.param[7]) or 0
		menu.mode = menu.param[8]
		menu.modeparam = menu.param[9] or {}
